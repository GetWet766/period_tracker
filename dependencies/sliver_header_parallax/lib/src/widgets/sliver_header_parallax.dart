...
class HeaderParallaxUtils {
  ...
  void checkAndSnap() {
    ...
    if (offset < midOffset) {
      // НАСТРОЙКА ПОРОГА:
      // 0.0 = нужно дотянуть совсем чуть-чуть, чтобы открылось.
      // 0.5 = нужно дотянуть до половины.
      // 0.8 = нужно дотянуть почти до самого верха (80% пути), иначе захлопнется.
      // По вашему запросу ставим низкий порог для offset (то есть нужно сильное усилие).

      // Логика: Если текущий оффсет больше, чем 25% от всего пути раскрытия
      // (значит мы НЕ дотянули до верха 75% пути), то возвращаем назад к Mid.
    ...
    // --- ЗОНА 2: Сворачивание (между Mid и Min) ---
